...
final class FinderController {
    ...
    private func slideOnscreen(edge: ScreenEdge) {
        ...
        let fallbackStart = currentFrame ?? edge.offscreenFrame(on: screen.frame, thickness: t, buffer: offscreenBuffer)

        guard let windowID = ensureManagedWindow(initialFrame: fallbackStart) else { return }

        // ensureManagedWindow may have adopted an existing front Finder window
        // and updated currentFrame to its on-screen bounds — animate from there
        // so the user's window glides in from where it is, not from offscreen.
        let start = currentFrame ?? fallbackStart

        // Position the managed window at the slide-start frame before Finder
        // becomes visible — otherwise unhiding shows it wherever it last sat
        // (often on an adjacent screen in multi-monitor setups).
        applyBounds(windowID: windowID, frame: start)
    ...
    private func parkOffscreen(edge: ScreenEdge) {
        ...
        animate(from: start, to: target, windowID: windowID, completion: { [weak self] in
            // Hide Finder so the parked window can't leak onto an adjacent
            // screen when the off-screen position falls inside another display.
            self?.hideFinderApp()
        })
        isParked = true
        currentFrame = target
    ...
    private func ensureManagedWindow(initialFrame: CGRect) -> Int? {
        ...
        // Reuse the front Finder window if the user already has one open —
        // otherwise their existing window stays put and we add a second one
        // (the original double-window bug on first hotkey after launch).
        if let info = frontFinderWindowInfo() {
            managedWindowID = info.id
            Defaults.managedWindowID = info.id
            currentFrame = info.cocoaFrame
            return info.id
        }
    ...
    private func frontFinderWindowInfo() -> (id: Int, cocoaFrame: CGRect)? {
        // AppleScript returns `false` (a non-list) when Finder has no windows;
        // numberOfItems == 5 only when we got the {id, x1, y1, x2, y2} list.
        guard let desc = run(FinderScript.frontWindowInfo()),
              desc.numberOfItems == 5,
              let idDesc = desc.atIndex(1),
              let x1Desc = desc.atIndex(2),
              let y1Desc = desc.atIndex(3),
              let x2Desc = desc.atIndex(4),
              let y2Desc = desc.atIndex(5),
              let primary = NSScreen.screens.first else { return nil }
        let id = Int(idDesc.int32Value)
        let x1 = CGFloat(x1Desc.int32Value)
        let y1 = CGFloat(y1Desc.int32Value)
        let x2 = CGFloat(x2Desc.int32Value)
        let y2 = CGFloat(y2Desc.int32Value)
        guard id != 0, x2 > x1, y2 > y1 else { return nil }
        let cocoaY = primary.frame.height - y2
        let frame = CGRect(x: x1, y: cocoaY, width: x2 - x1, height: y2 - y1)
        return (id, frame)
    }
    ...
    private func activateAndFrontManagedWindow(windowID: Int) {
        ...
        // `tell application "Finder" activate` (the prior approach) opens a
        // fresh Finder window whenever Finder thinks it has no visible front
        // window — which is exactly the state we leave it in after parking.
        // NSRunningApplication.activate unhides and frontmosts without that
        // side effect, then we use AppleScript only to raise *our* window.
        if let finder = finderApp() {
            finder.unhide()
            finder.activate(options: [])
        }
        run(FinderScript.bringWindowFront(windowID: windowID))
    }

    private func hideFinderApp() {
        finderApp()?.hide()
    }
...
private enum FinderScript {
    static func createWindow(at r: CGRect) -> String {
        // Finder visibility is managed via NSRunningApplication on the Swift
        // side, not by toggling System Events here — mixing the two left
        // Finder in a state where the next `activate` would spawn an extra
        // window.
        """
    ...
    static func bringWindowFront(windowID: Int) -> String {
        """
        tell application "Finder"
            try
    ...
    static func frontWindowInfo() -> String {
        """
        tell application "Finder"
            try
                ...
                return {id of w as integer, item 1 of b, item 2 of b, item 3 of b, item 4 of b}
            on error
                return false
            end try
        end tell
